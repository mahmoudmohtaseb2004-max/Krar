...
# ==================== الإعدادات ====================
TOKEN = "8770091738:AAFEcZuqJfs6jfloBq1y5lwZgNaRnwi11Fg"
...
def save_user(user_id):
    r.sadd(f"{REDIS_PREFIX}:users", user_id)

def get_all_users():
    return list(r.smembers(f"{REDIS_PREFIX}:users"))

def get_users_count():
    return r.scard(f"{REDIS_PREFIX}:users")

def get_messages_count():
    return len(r.keys(f"{REDIS_PREFIX}:msg:*"))
...
def ban_user(user_id):
    r.sadd(f"{REDIS_PREFIX}:banned", user_id)

def unban_user(user_id):
    r.srem(f"{REDIS_PREFIX}:banned", user_id)

def is_banned(user_id):
    return r.sismember(f"{REDIS_PREFIX}:banned", str(user_id))

def get_banned_count():
    return r.scard(f"{REDIS_PREFIX}:banned")
